    ...
    while True:
        ...
        while retriesLogic(False):
            ...
            if hashedPsw == storedPsw:
                ...
                user = User(userName=userName, AESkey=c.deriveAESkey(str(inPsw.decode(encoding)), str(salt)),
                            index=index)
                return user, True
            else:
                popUp(
                    "> Contraseña incorrecta!")  # ToDo revisar que no se borre el nombre de usuario cuando le erro de contrasena
    ...
    while True:
        ...
        if checkMsg == True:
            ...
            if password != password2:
                popUp(
                    " > Contraseñas no coinciden!")  # ToDo revisar que no se borre el nombre de usuario cuando le erro de contrasena
                continue
            break
        else:
            popUp(checkMsg)
